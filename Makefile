...
# bundled gcc (note, this is not linked to the main program so does not make the main program a derived work under GPL) [GPL]
# bundled binutils (the same license applies) [GPL]
# avrdude [GPL]

# bundled swiftc from Apple (see license)
# in the future we should build and ship our own swiftc
...
# define ADDITIONAL_OBJECTS to include other C or C++ files to be built, they must be of the form %.cpp.o or $.c.o.
...
ifeq ($(COMPRESS_SIZE_OPTIMISATIONS),yes)
...
SYSTEM_SWIFTC = $(wildcard /usr/bin/swiftc)
...
clean:
	...
	*.swiftdoc *.swiftsourceinfo *.swiftinterface lib*.a $(OUTPUT_MAP_FILE) project.include
...
$(ELF_FILE) $(ELF_LINKER_OUTPUT_FILE) : $(FULL_BUILD_PATH) $(ALL_SWIFT_OBJECTS) $(ADDITIONAL_OBJECTS) $(ELF_DEPS) $(AUTOLINK_FILE)
	@echo $(ELF_BUILD_STATUS)
	$(LD) -o $(ELF_FILE) $(ALL_SWIFT_OBJECTS) $(ADDITIONAL_OBJECTS) $(TEXT_SECTION_SIZE) $(DATA_SECTION_SIZE) $(MISSING_RUNTIME_STUB) \
...
$(FULL_BUILD_PATH)lib$(MODULE_NAME).a: $(FULL_BUILD_PATH) $(ALL_SWIFT_OBJECTS) $(ADDITIONAL_OBJECTS)
	@echo "** Create library $(MODULE_NAME)"
	set -o pipefail && $(AR) -o $@ $(ALL_SWIFT_OBJECTS) $(ADDITIONAL_OBJECTS) $(APPEND_BUILD_LOG)
...
-include $(patsubst %,$(FULL_BUILD_PATH)%,$(ADDITIONAL_OBJECTS:.o=.d))
...
$(FULL_BUILD_PATH)%.cpp.bc $(FULL_BUILD_PATH)%.cpp.d: %.cpp
	$(CLANG_PP) -emit-llvm -MMD -MF$(<:%.cpp=$(FULL_BUILD_PATH)%.cpp.d) -c $(EXTRA_CPP_PARAMS) -o $(<:%.cpp=$(FULL_BUILD_PATH)%.cpp.bc) $<
	echo "Emitted $(<:%.cpp=$(FULL_BUILD_PATH)%.cpp.bc)"

$(FULL_BUILD_PATH)%.c.bc $(FULL_BUILD_PATH)%.c.d: %.c
	$(CLANG) -emit-llvm -MMD -MF$(<:%.c=$(FULL_BUILD_PATH)%.c.d) -c $(EXTRA_C_PARAMS) -o $(<:%.c=$(FULL_BUILD_PATH)%.c.bc) $<
	echo "Emitted $(<:%.c=$(FULL_BUILD_PATH)%.c.bc)"
...
.PHONY : reboot getfuses getsig geteeprom import_to_avr download_from_avr burn_bootloader reopen sil csil
